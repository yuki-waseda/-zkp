...
template Main (out_dim, in_dim, S_clip, sigma) {
  ...
  // ビット列による正規分布の近似
  var binomial_mean = 16;
  signal binomial_sum[out_dim][in_dim][32]; 
  signal centered_binomial_sum[out_dim][in_dim];
   for(var i =0; i < out_dim; i++){
     for(var j = 0; j < in_dim; j++){
        for(var k = 0; k < 32; k++){
            binomial_sum[i][j][k] <== (k == 0) ? randSeq[k + 32 * (in_dim * i + j)] : binomial_sum[i][j][k-1] + randSeq[k + 32 * (in_dim * i + j)];
        }
        centered_binomial_sum[i][j] <== binomial_sum[i][j][32-1] - binomial_mean;
     }
  }
